...
#define TILE_WIDTH 32 // tile witdt 
...
__global__ void convolution(int *d_mat,int *d_kernel,unsigned int number_rows ,int d,int *output_mat,int h,int w,int C){
    ...
    */
    int t_idy = blockDim.x*blockIdx.x + threadIdx.x;// for the number of the element being changed
    int t_idx = blockDim.y*blockIdx.y + threadIdx.y;// for the number of kernels
...
//////// another convolution function ////////// ------------------ using shared memory

__global__ void convolution_opti(int *d_mat,int *d_kernel,unsigned int *number_rows,int d,int *output_mat,int h,int w,int C){

    /* Kernel to perform matrix multiplication. We utilize shared memory and tiling to perform the matrix multiplication. */

    int tx = threadIdx.x;  /*Thread-ID in the x-direction */
    int ty = threadIdx.y;  /*Thread-ID in the y-direction */
    __shared__ int image_mat_s[TILE_WIDTH][TILE_WIDTH]; /* Shared memory to be used by threads in a block */    
    __shared__ int kernel_mat_s[TILE_WIDTH][TILE_WIDTH]; /// Since we have multiple kernels to taken into account at the ame time 

    int row = blockIdx.y*blockDim.y + ty; /* row in the output matrix */
    int col = blockIdx.x*blockDim.x + tx; /* column in the output matrix */
    int pSum = 0;

    for(int m=0;m<(w*h*C+TILE_WIDTH-1)/TILE_WIDTH;m++){
        if(row<*number_rows && (m*TILE_WIDTH+tx)<(w*h*C) )     image_mat_s[ty][tx] = d_mat[row*w*h*C+m*TILE_WIDTH+tx];
        if( (ty+m*TILE_WIDTH)<(w*h*C) && col<d )      kernel_mat_s[ty][tx] = d_kernel[(ty+m*TILE_WIDTH)*d+col]; /* This is assuming that the tile is a sqaure */
        __syncthreads(); 
        for(int i=0;i<TILE_WIDTH;i++){
            pSum += image_mat_s[ty][i]*kernel_mat_s[i][tx];
        }
        __syncthreads();
        image_mat_s[ty][tx] = 0; /* Setting the elements in the shared memory back to 0. This takes care of the corner cases where junk values are stored */
        kernel_mat_s[ty][tx] = 0;  
    }
    if(row<*number_rows && col<d)    output_mat[row*d+col] = pSum; /* Load the result into the output matrix */
}
...
__global__ void scatter(int *output_mat, int *d_row_map, unsigned int *number_rows, int *output,int H,int W,int d,int h,int w){
    ...
    */
    ...
    int t_idy = blockIdx.x*blockDim.x + threadIdx.x;// The number of convs in the output matrux
    int t_idx = blockDim.y*blockIdx.y + threadIdx.y;// The number of output kernels
    // printf("%d,%d,%d \n",t_idx,t_idy, 0);
    if(t_idx<*number_rows && t_idy <d){
...
int main(){
    ...
    */
    ...
    for(int j=0; j<n_streams; j++){
        ...
        //convolution kernel
        convolution_opti<<<Block_c, Thread_c,0, streams[j]>>>(&d_mat[mat_offset], d_kernel, &row_address[j], d, &output_mat[om_offset],h,w,C);
